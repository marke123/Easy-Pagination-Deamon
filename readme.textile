...
(Mabye) To come within the next versions: Works on single post pages too.
...
It's prepared for a language file that translates "First", "Last" & the error messages,
...
* _v0.1.4_ Exchanged the `$range` argument with an array of `$args` to allow additional classes for `<li>` elements
